...
const Room: FunctionComponent<RoomProps> = ({  }) => {
    ...
    const joinRoom = () => {
        RoomService.JoinRoom(roomName, roomPassword)
        .then(res => {
            if(res.ok) {
                res.json().then(res => {
                    if(rooms) {
    ...
    return (
        <Container maxWidth='md' sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', p: '16px'}} >
            <Box sx={{borderRadius: '4px', backgroundColor: '#1D1B54', display: 'flex', flexDirection: 'column', alignItems: 'center', py: '16px', width: '100%'}}>
                ...
                <CssTextField key={'roomName'} id="outlined-basic" label="Room name" variant="outlined" value={roomName} onChange={(e) => setRoomName(e.target.value)} />
                <CssTextField key={'roomPassword'} id="outlined-basic" label="Password" variant="outlined" value={roomPassword} onChange={(e) => setRoomPassword(e.target.value)} />
                <Box sx={{display: 'flex', justifyContent: 'space-evenly', width: '100%'}}>
                    <StyledButton variant="outlined" size="large" onClick={joinRoom}>Join</StyledButton>
                    <StyledButton variant="outlined" size="large" onClick={createRoom}>Create</StyledButton>
                </Box>
                {
                ...
                {rooms && rooms.length > 0 &&
                    <Box sx={{width: 'calc(100% - 32px)'}}>
                        ...
                        {rooms.map((room, index) =>
                            <Box key={room.id} sx={{display: 'flex', justifyContent: 'center', flexDirection: 'column'}}>
                                <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
                                    ...
                                    <StyledButton variant="outlined" size="large" onClick={() => leaveRoom(room.id)}>Leave</StyledButton>
                                </Box>
